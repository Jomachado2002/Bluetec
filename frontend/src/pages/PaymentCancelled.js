...
const PaymentCancelled = () => {
  ...
  const contactWhatsApp = () => {
    const message = `Hola! Tuve un problema al procesar mi pago en BlueTec.%0A%0AID de transacción: ${paymentInfo.shop_process_id}%0AMonto: ${displayPYGCurrency(paymentInfo.amount)}%0AMotivo: Pago cancelado%0A%0A¿Pueden ayudarme a completar mi compra? Gracias!`;
    window.open(`https://wa.me/595981150393?text=${message}`, '_blank');
  };

  const sendEmail = () => {
    const subject = `Problema con pago - ${paymentInfo.shop_process_id}`;
    const body = `Estimados,%0A%0ATuve un problema al procesar mi pago en BlueTec:%0A%0AID de transacción: ${paymentInfo.shop_process_id}%0AMonto: ${displayPYGCurrency(paymentInfo.amount)}%0AMotivo: Pago cancelado%0A%0APor favor ayúdenme a completar mi compra.%0A%0AGracias!`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 to-gray-50 py-8">
      <div className="container mx-auto px-4 max-w-2xl">
        ...
        <div className="mt-8 text-center text-sm text-gray-500">
          <p>
            No te preocupes, estamos aquí para ayudarte • BlueTec - Tu tecnología de confianza
          </p>
        </div>
      </div>
    </div>
